...
class ViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        let centering: Int = Int(view.frame.width)/2 - (numcells * cellsize + (spacing * 4))/2
        
        for rows in 0...(numcells-1) {
            for cols in 0...(numcells-1) {
                
                let back = UIView()
                let eachcell = UIView()
                
                back.frame = CGRect(x: centering + (rows * cellsize) + (spacing * rows), y: 100 + (cols * cellsize) + (spacing * cols), width: cellsize, height: cellsize)
                eachcell.frame = back.bounds
                eachcell.backgroundColor = randomColor()
                
                textlabel = UILabel(frame: eachcell.bounds)
                textlabel.textAlignment = .center
                textlabel.text = "\(rows, cols)"
                
                //                cellView.addSubview(textlabel)
                
                eachcell.layer.borderWidth = 1.0
                eachcell.layer.borderColor = UIColor.gray.cgColor
                eachcell.layer.cornerRadius = 8
                eachcell.layer.masksToBounds = true;
    ...
    @objc func swipes(swiped: UIPanGestureRecognizer) {
        ...
        let centerDistance: CGFloat = 20.0
        let maximumDistance: CGFloat = 100.0
        ...
        case .changed:
            ...
            } else if centerDistance < distance && distance < maximumDistance {
